                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01"
                                    ...
                                    value="{{ old('total', $expense->total) }}" required>
                                @error('total')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="month" class="form-control @error('month') is-invalid @enderror" id="month"
                                    name="month" value="{{ old('month', $expense->month?->format('Y-m')) }}" required>
                                @error('month')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 d-flex align-items-center">
                                <div id="sheet-preview" class="w-100 {{ $expense->sheet ? '' : 'd-none' }}">
                                    <div class="border rounded-3 p-3 d-flex align-items-center gap-3">
                                        ...
                                        <div>
                                            <p class="mb-0 fw-semibold" id="sheet-name">
                                                {{ $expense->sheet?->original_name ?? 'Selected file' }}</p>
                                            <small class="text-muted" id="sheet-size">
                                                @if ($expense->sheet)
                                                    {{ number_format($expense->sheet->size / 1024, 1) }} KB
                                                @endif
                                            </small>
                                        </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const setOriginal = () => {
                @if ($expense->sheet)
                    nameEl.textContent = '{{ $expense->sheet->original_name }}';
                    sizeEl.textContent = '{{ number_format($expense->sheet->size / 1024, 1) }} KB';
